...
export class SimklProvider implements Provider {
  ...
  private client(credentials: Credentials): HttpClient {
    ...
    if (!client) {
      client = new HttpClient('https://api.simkl.com', {
        headers: { Authorization: `Bearer ${credentials.token}`, 'User-Agent': 'AIOSync/1.1.3', 'Content-Type': 'application/json' },
  ...
  private path(path: string, params: Record<string, string> = {}): string {
    return `${path}?${new URLSearchParams({ client_id: this.clientId, 'app-name': 'AIOSync', 'app-version': '1.1.3', ...params })}`;
  }
